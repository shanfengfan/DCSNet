...
class FasterRCNNUNet(nn.Module):
    ...
    def compute_seg_loss(self, pred_dict, proposals, targets):
        ...
        for img_idx, boxes in enumerate(proposals):
            ...
            img_h, img_w = target_bin.shape[-2:]
            for box in boxes:
        ...
        if self.training or (targets is not None):
            ...
            if rois_list:
                ...
                if self.training:
                    ...
                    roi_boxes = self.apply_roi_jitter(roi_boxes, (img_h, img_w), jitter_ratio=0.02)
...
def compute_fused_multi_scale_loss(pred_dict, seg_targets, loss_weights=None,
    ...
    def focal_loss(pred, target, alpha: float = 0.35, gamma: float = 2.0):
...
def map_roi_logits_to_full_image(seg_logits, proposals, full_img_size):
...
def map_roi_seg_to_full_image(seg_logits, proposals, full_img_size, threshold=0.5):
    device = seg_logits.device if seg_logits is not None else torch.device('cpu')
    batch_size = len(proposals)
